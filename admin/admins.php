...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .role-badge { display:inline-block; padding:2px 10px; border-radius:20px; font-size:0.75rem; font-weight:600; margin-left:8px; }
    .stats-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:20px; margin-bottom:32px; }
    .stat-card { background:#fff; border-radius:12px; padding:24px 20px; box-shadow:0 2px 8px rgba(0,0,0,0.06); display:flex; align-items:center; gap:16px; }
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="messages.php"><i class="fas fa-envelope"></i> Messages
      <?php if ($total_messages > 0): ?>
        <span class="unread-badge"><?= $total_messages ?></span>
      <?php endif; ?>
    </a>
    <?php if (isSuperAdmin()): ?>
  ...
  <div class="sidebar-footer">
    ...
    <span class="role-badge" style="background:<?= isSuperAdmin() ? '#fff3cd' : '#d1ecf1' ?>;color:<?= isSuperAdmin() ? '#856404' : '#0c5460' ?>">
  ...
  <div class="page-header">
    <h2>Dashboard
      <?php if (isset($_GET['error'])): ?>
        <span style="color:#e74c3c;font-size:0.85rem;margin-left:8px;"><i class="fas fa-exclamation-triangle"></i> Unauthorized access</span>
      <?php endif; ?>
    </h2>
